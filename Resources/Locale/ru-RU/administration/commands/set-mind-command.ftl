cmd-setmind-desc = Перемещает разум в указанную сущность. Сущность должна иметь { $requiredComponent }. По умолчанию это заставит разум, который в данный момент посещает другие сущности, вернуться обратно (например, вернуть призрака в его основное тело).
cmd-setmind-help = Использование: { $command } <entityUid> <username> [unvisit]
cmd-setmind-command-target-has-no-content-data-message = У целевого игрока отсутствуют данные содержимого (что за фигня?)
